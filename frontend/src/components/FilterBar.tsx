...
export const FilterBar = ({ filters, onFiltersChange, categories }: FilterBarProps) => {
  ...
  return (
    <div id="places-section" className="sticky top-20 z-40 mb-12">
      {/* Glass Container */}
      <div className="bg-white/10 backdrop-blur-xl rounded-3xl border border-white/20 p-6 shadow-2xl">
        <div className="space-y-6">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search places, notes, or neighborhoods..."
              value={filters.search || ''}
              onChange={(e) => handleSearchChange(e.target.value)}
              className="w-full pl-12 pr-4 py-4 bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl text-white placeholder-white/60 focus:bg-white/15 focus:border-white/40 focus:ring-2 focus:ring-purple-400/30 focus:outline-none transition-all duration-300"
            />
            {filters.search && (
              <button
              ...
              {categories.map((category) => {
                ...
                return (
                  <button
                    ...
                    className={`group relative px-6 py-3 rounded-2xl font-semibold transition-all duration-300 hover:scale-105 capitalize ${
                      isActive
                  ...
                  >
                    ...
                    {isActive && (
                      <div className={`absolute inset-0 bg-gradient-to-r ${config.color} rounded-2xl blur opacity-20`} />
                    )}
                  </button>
                );
              })}
